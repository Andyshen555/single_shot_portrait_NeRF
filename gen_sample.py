...
def generate_images(
    ...
    print('Loading eg3d from "%s"...' % network_pkl)
    device = torch.device('cuda')
    with dnnlib.util.open_url(network_pkl) as f:
        G = legacy.load_network_pkl(f)['G_ema'].to(device) # type: ignore

    # Specify reload_modules=True if you want code modifications to take effect; otherwise uses pickled code
    print("Reloading eg3d!")
    ...
    print("Loading lp3d")
    model = lp3d(True).to(device).eval()
    ...
    with torch.no_grad():
        for seed_idx, seed in enumerate(seeds):
            ...
            for i in range(cnt):
                ...
                eg_mapping = G.mapping(z, conditioning_params, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff) # latent vector
                ...
                img = (lp_img.permute(0, 2, 3, 1) * 127.5 + 128).clamp(0, 255).to(torch.uint8)
                imgs.append(img)

            img = torch.cat(imgs, dim=2)

            PIL.Image.fromarray(img[0].cpu().numpy(), 'RGB').save(f'{outdir}/lp3d_seed{seed:04d}.png')
